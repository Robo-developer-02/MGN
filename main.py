...
============================================================
  🤖 Speech-to-Speech AI Chatbot — Powered by Groq
============================================================
  Stack:
    STT  → Groq Whisper (whisper-large-v3-turbo)
    LLM  → Groq Chat model (streamed, sentence-by-sentence)
  ...
  Optimisations in this version:
    ...
    3. Prompt caching   — (a) exact-match reply cache so a repeated
       ...
       system+history prefix is kept stable so Groq's own server-side
...
from groq import (
    Groq,
    APIError,          # base class — catches every Groq SDK error
...
# Groq Whisper endpoint. "whisper-large-v3-turbo" is faster/cheaper;
...
# every future "GPT First Token" latency -- exactly the kind of thing that
...
# sent to the Groq Whisper API -- a noise burst used to still get shipped
...
# Whisper (verbose_json) returns per-segment no_speech_prob / avg_logprob
# for free -- this data was already coming back, just unused. Used to drop
# text Whisper itself flagged as low-confidence noise. Zero extra latency,
# zero extra API calls, and it can only ever remove noise -- it can't reject
# real speech Whisper was confident about.
...
SYSTEM_EN = (
    "Your name is Naaila. You are a helpful AI assistant created by Robotwala. "
...
SYSTEM_HI = (
    "Aapka naam Naaila hai. Aap Robotwala dwara banaya gaya helpful AI assistant hain. "
...
# ordered prefix (system + history) on every call so Groq's server-side
...
def handle_error(e: Exception, where: str):
    ...
    Central error classifier. Order matters:
      ...
      2. Internet is fine but the Groq
...
def transcribe_segment(audio: np.ndarray) -> Tuple[str, str]:
    """Groq Whisper call for a single audio segment. Returns (text, lang).
    ...
    # Reject low-confidence "noise" transcriptions using per-segment
    # no_speech_prob / avg_logprob that verbose_json already returns --
    # no extra API call. This can only ever remove text Whisper itself
    # flagged as unlikely to be real speech; it can't reject a segment
    # Whisper was actually confident about.
    ...
    if segments and text:
        ...
        avg_no_speech = sum(_seg_get(s, "no_speech_prob") for s in segments) / len(segments)
        avg_logprob   = sum(_seg_get(s, "avg_logprob") for s in segments) / len(segments)
...
def stream_ai_reply_and_speak(user_text: str, lang: str) -> str:
    ...
    # ── Stable prefix (system + history) so Groq's server-side
    ...
    if full_reply:                        # feature 4 — don't cache/store empties
        ...
        # Cap history so prompt size — and therefore GPT First Token latency
...
def main():
    ...
    try:speak_blocking('Hi , I am RoboBot. Aapka swagat hai. Aap apna sawaal poochhiye.', lang="hi")
    except Exception as e:
